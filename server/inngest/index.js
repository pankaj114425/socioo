...
//inggest  function to used for to save user data to a database
...
// inggest function to for update user data in database
...
//inggest function to for delete user data in database
...
//ingest function to send reminder when a new connection request is added
    ...
    await step.run('send-connection-request-email',async()=>{
      ...
      const body = `
          <div style="font-family: Arial, sans-serif; padding: 20px;">
            ...
            <br/>
            <p>Thanks,<br/>Socioo - Stay Connected</p>
          </div>
          `;
    await sendEmail(
 ...
 //innget function to delete story after 24 hours
...
//   {cron:"TZ=America/new_york 0 9 * * *"}  //everday at 9 AM
//    async({step})=>{
...
const sendNotificationOfUnseenMessages = inngest.createFunction(
  ...
  async ({ step }) => {
    ...
    // Step 2: Count unseen messages per user
    const unseenCount = {};
    ...
    // Step 3: Send email notification to each user
    for (const userId in unseenCount) {
      const user = await User.findById(userId);
...
export const functions = [syncUserCreation,
              ...
              deleteStory,sendNotificationOfUnseenMessages];
