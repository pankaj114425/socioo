  ...
  return (
    ...
    text-white flex items-center justify-center p-4'>
       <div className='w-full max-w-md'>
           ...
           <div className='rounded-lg h-96 flex items-center justify-between relative '
             style={{backgroundColor:background}}> 

             {mode === 'text' &&
              <textarea className='bg-transparent text-white w-full h-full p-6
              text-lg resize-none focus:outline-none' placeholder="What's in your mind"
            ...
            
            {
              mode === 'media' && previewUrl && (
                media?.type.startsWith('image') ?(
                   <img src={previewUrl} alt="" className='object-contain w-full max-h-full' />
                ):(
                  <video src={previewUrl} className='object-contain w-full max-h-full '/>
                )
              )
            }
           </div>
           <div className='flex mt-4 gap-2'>
              ...
              <div className='flex gap-2 mt-4'>
                    <button onClick={()=>{setMode('text');setMedia(null);setPreviewUrl(null)}} className={`flex-1 flex items-center justify-center gap-2 cursor-pointer
                      p-2 rounded ${mode === 'text' ? "bg-white text-black" :"bg-zinc-800" }`}>
                      <TextIcon size={18}/>Text
                    </button>
                    <label className={`flex-1 flex item-center justify-center gap-2 p-2
               ...
               {
                loading:'Saving...',
                success:'Story successfuly added',
                error: e=> {e.message}
               })} className='w-full flex  items-center justify-center gap-2 text-white py-3 mt-4 rounded
