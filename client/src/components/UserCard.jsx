     ...
     border-gray-200 rounded-md'>
        <div className='text-center'>
          <img src={user.profile_picture} alt="" className='rounded-full w-16
           ...
           {user.bio && <p className='text-gray-600 mt-2 text-center text-sm'>{user.bio}</p>}
        </div>
        ...
        <div className='flex mt-4 gap-2'>
              {/* follow button */}
              <button onClick={handleFollow} disabled={currentUser.following.includes(user._id)} className='w-full py-2 rounded-md flex justify-center items-center
              gap-2 bg-gradient-to-r from-indigo-500 to-purple-600
          ...
          <button className='flex items-center justify-center w-16 border
          text-slate-500 group rounded-md cursor-pointer active:scale-95 transition
          '>
            {currentUser?.connections.includes(user._id) ?
            <MessageCircle  onClick={handleConnectionRequest} className='w-5 h-5 group-hover:scale-105 transition'/> :
